...
import { Globe, AtSign, Link2, ArrowUpRight, Sparkles } from "lucide-react";
import { labName, labNameLong } from "@/lib/brand";
...
const columns = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: "Evaluations", href: "/#benchmarks" },
    ],
  },
  {
    title: "Company",
...
const socials = [
  { icon: Globe, href: "https://github.com/minimax", label: "GitHub" },
  { icon: AtSign, href: "https://twitter.com/minimax", label: "X" },
  { icon: Link2, href: "https://linkedin.com/company/minimax", label: "LinkedIn" },
];

export function Footer() {
  return (
    <footer
      className="relative mt-24 border-t border-border bg-grid"
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
            ...
            <div className="mt-6 flex items-center gap-3">
              {socials.map((s) => (
                <Link
                  ...
                  rel="noreferrer"
                  aria-label={s.label}
                  className="flex size-9 items-center justify-center rounded-full border border-border bg-card text-muted-foreground transition-colors hover:border-accent/50 hover:text-foreground"
                >
        ...
        <div className="mt-14 flex flex-col items-center justify-between gap-4 border-t border-border pt-8 text-xs text-muted-foreground sm:flex-row">
          <p>&copy; {new Date().getFullYear()} {labNameLong}. All rights reserved.</p>
...
export function BrandMark({ className = "" }: { className?: string }) {
  return (
    ...
    >
      <defs>
        <linearGradient id="minimax-mark" x1="0" y1="0" x2="32" y2="32">
          <stop offset="0%" stopColor="var(--brand-1)" />
          <stop offset="50%" stopColor="var(--brand-2)" />
          <stop offset="100%" stopColor="var(--brand-3)" />
        </linearGradient>
      </defs>
      <circle cx="16" cy="16" r="11" stroke="url(#minimax-mark)" strokeWidth="1.6" opacity="0.9" />
      <ellipse cx="16" cy="16" rx="11" ry="4.5" stroke="url(#minimax-mark)" strokeWidth="1.6" transform="rotate(60 16 16)" />
      <ellipse cx="16" cy="16" rx="11" ry="4.5" stroke="url(#minimax-mark)" strokeWidth="1.6" transform="rotate(-60 16 16)" />
      <circle cx="16" cy="16" r="3" fill="url(#minimax-mark)" />
    </svg>
  );
}